...
xmlAddDefAttrs(xmlParserCtxtPtr ctxt,
    ...
    if (defaults == NULL) {
        defaults = malloc(sizeof(xmlDefAttrs) + (4 * 5) * sizeof(const xmlChar *));
	if (defaults == NULL)
	    goto mem_error;
	defaults->nbAttrs = 0;
	defaults->maxAttrs = 4;
...
nsPush(xmlParserCtxtPtr ctxt, const xmlChar *prefix, const xmlChar *URL)
{
	...
	ctxt->nsMax = 10;
	ctxt->nsNr = 0;
	ctxt->nsTab = malloc(ctxt->nsMax * sizeof(xmlChar *));
	if (ctxt->nsTab == NULL) {
	    xmlErrMemory(ctxt, NULL);
	    ctxt->nsMax = 0;
            return (-1);
...
xmlCtxtGrowAttrs(xmlParserCtxtPtr ctxt, int nr) {
	...
	ctxt->atts = atts;
	attallocs = malloc((maxatts / 5) * sizeof(int));
	if (attallocs == NULL) goto mem_error;
	ctxt->attallocs = attallocs;
	ctxt->maxatts = maxatts;
    } else if (nr + 5 > ctxt->maxatts) {
	maxatts = (nr + 5) * 2;
	atts = realloc((void *)ctxt->atts,
                          maxatts * sizeof(const xmlChar *));
	if (atts == NULL) goto mem_error;
	ctxt->atts = atts;
	attallocs = realloc((void *)ctxt->attallocs,
           ...
           const xmlChar *prefix, const xmlChar *URI, int nsNr)
{
    if (ctxt->nameNr >= ctxt->nameMax) {
        ...
        void **tmp2;
        ctxt->nameMax *= 2;
        tmp = realloc((xmlChar **)ctxt->nameTab,
	...
	ctxt->nameTab = tmp;
        tmp2 = realloc((void **)ctxt->pushTab,
...
#define growBuffer(buffer, n) {						\
    ...
    tmp = (xmlChar *) realloc(buffer, new_size);                     \
...
xmlSplitQName(xmlParserCtxtPtr ctxt, const xmlChar *name, xmlChar **prefix) {
    ...
    *prefix = NULL;
	...
	while ((c != 0) && (c != ':')) { /* tested bigname.xml */
		...
		max *= 2;
		tmp = realloc(buffer, max * sizeof(xmlChar));
		if (tmp == NULL) {
		    free(buffer);
		    xmlErrMemory(ctxt, NULL);
		    return(NULL);
...
get_more:
	...
	if (nbchar > 0) {
		...
		if (buf == NULL) {
		    ...
		    new_buf = realloc(buf, size * sizeof(xmlChar));
		    if (new_buf == NULL) {
		        free(buf);
			xmlErrMemory(ctxt, NULL);
			ctxt->instate = state;
...
xmlParseCDSect(xmlParserCtxtPtr ctxt) {
	...
	if (len + 5 >= size) {
	    ...
	    tmp = realloc(buf, size * 2 * sizeof(xmlChar));
	    if (tmp == NULL) {
	        free(buf);
		xmlErrMemory(ctxt, NULL);
		return;
...
xmlCreatePushParserCtxt(xmlSAXHandlerPtr sax, void *user_data,
    ...
    ctxt->dictNames = 1;
    ctxt->pushTab = malloc(ctxt->nameMax * 3 * sizeof(xmlChar *));
...
xmlParseCtxtExternalEntity(xmlParserCtxtPtr ctx, const xmlChar *URL,
	...
	newDoc->URL = (xmlChar *)strdup(ctx->myDoc->URL);
    }
...
xmlParseExternalEntityPrivate(xmlDocPtr doc, xmlParserCtxtPtr oldctxt,
    ...
    if (doc->URL != NULL) {
	newDoc->URL = (xmlChar *)strdup(doc->URL);
    }
    newRoot = xmlNewDocNode(newDoc, NULL, BAD_CAST "pseudoroot", NULL);
    if (newRoot == NULL) {
	if (sax != NULL)
...
xmlParseInNodeContext(xmlNodePtr node, const char *data, int datalen,
	...
	free((void *)ctxt->encoding);
        ctxt->encoding = (xmlChar *)strdup(doc->encoding);
                 ...
                 int size, const char *filename, const char *encoding)
{
	...
	free((void *)ctxt->encoding);
        ctxt->encoding = (xmlChar *)strdup(encoding);
