...
xmlRegEpxFromParse(xmlRegParserCtxtPtr ctxt) {
...
#endif
	transitions = malloc((nbstates + 1) * (nbatoms + 1) * sizeof(int));
	if (transitions == NULL) {
	    free(stateRemap);
	    free(stringRemap);
	    free(stringMap);
	...
	for (i = 0;i < ret->nbStates;i++) {
		...
		if ((trans->atom->data != NULL) && (transdata == NULL)) {
		    transdata = malloc(nbstates * nbatoms * sizeof(void *));
		    if (transdata != NULL)
			memset(transdata, 0,
...
xmlRegNewParserCtxt(const xmlChar *string) {
    ...
    ret = malloc(sizeof(xmlRegParserCtxt));
    if (ret == NULL)
	return(NULL);
...
xmlRegCopyAtom(xmlRegParserCtxtPtr ctxt, xmlRegAtomPtr atom) {
    ...
    if (atom->nbRanges > 0) {
        ...
        ret->ranges = malloc(sizeof(xmlRegRangePtr) * atom->nbRanges);
	if (ret->ranges == NULL) {
	    xmlRegexpErrMemory(ctxt, "copying atom");
	    goto error;
	}
...
xmlRegAtomAddRange(xmlRegParserCtxtPtr ctxt, xmlRegAtomPtr atom,
    ...
    if (atom->maxRanges == 0) {
	atom->maxRanges = 4;
	atom->ranges = malloc(atom->maxRanges * sizeof(xmlRegRangePtr));
	if (atom->ranges == NULL) {
	...
	xmlRegRangePtr *tmp;
	atom->maxRanges *= 2;
	tmp = realloc(atom->ranges,
                         atom->maxRanges * sizeof(xmlRegRangePtr));
	if (tmp == NULL) {
	    xmlRegexpErrMemory(ctxt, "adding ranges");
	    atom->maxRanges /= 2;
	    return;
...
xmlRegGetCounter(xmlRegParserCtxtPtr ctxt) {
	...
	ctxt->maxCounters = 4;
	ctxt->counters = malloc(ctxt->maxCounters * sizeof(xmlRegCounter));
	...
	xmlRegCounter *tmp;
	ctxt->maxCounters *= 2;
	tmp = realloc(ctxt->counters,
                         ctxt->maxCounters * sizeof(xmlRegCounter));
	if (tmp == NULL) {
	    xmlRegexpErrMemory(ctxt, "allocating counter");
	    ctxt->maxCounters /= 2;
	    return(-1);
...
xmlRegAtomPush(xmlRegParserCtxtPtr ctxt, xmlRegAtomPtr atom) {
    ...
    if (ctxt->maxAtoms == 0) {
	ctxt->maxAtoms = 4;
	ctxt->atoms = malloc(ctxt->maxAtoms * sizeof(xmlRegAtomPtr));
	if (ctxt->atoms == NULL) {
	...
	xmlRegAtomPtr *tmp;
	ctxt->maxAtoms *= 2;
	tmp = realloc(ctxt->atoms, ctxt->maxAtoms * sizeof(xmlRegAtomPtr));
	if (tmp == NULL) {
	    xmlRegexpErrMemory(ctxt, "allocating counter");
	    ctxt->maxAtoms /= 2;
	    return(-1);
...
xmlRegStateAddTransTo(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr target,
	...
	target->maxTransTo = 8;
	target->transTo = malloc(target->maxTransTo * sizeof(int));
	...
	int *tmp;
	target->maxTransTo *= 2;
	tmp = realloc(target->transTo, target->maxTransTo * sizeof(int));
	if (tmp == NULL) {
	    xmlRegexpErrMemory(ctxt, "adding transition");
	    target->maxTransTo /= 2;
	    return;
...
xmlRegStateAddTrans(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr state,
    ...
    if (state->maxTrans == 0) {
	state->maxTrans = 8;
	state->trans = malloc(state->maxTrans * sizeof(xmlRegTrans));
	if (state->trans == NULL) {
	    xmlRegexpErrMemory(ctxt, "adding transition");
	    state->maxTrans = 0;
	    return;
	...
	xmlRegTrans *tmp;
	state->maxTrans *= 2;
	tmp = realloc(state->trans, state->maxTrans * sizeof(xmlRegTrans));
	if (tmp == NULL) {
	    xmlRegexpErrMemory(ctxt, "adding transition");
	    state->maxTrans /= 2;
	    return;
...
xmlRegStatePush(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr state) {
    ...
    if (ctxt->maxStates == 0) {
	ctxt->maxStates = 4;
	ctxt->states = malloc(ctxt->maxStates * sizeof(xmlRegStatePtr));
	if (ctxt->states == NULL) {
	...
	xmlRegStatePtr *tmp;
	ctxt->maxStates *= 2;
	tmp = realloc(ctxt->states,
                         ctxt->maxStates * sizeof(xmlRegStatePtr));
	if (tmp == NULL) {
	    xmlRegexpErrMemory(ctxt, "adding state");
	    ctxt->maxStates /= 2;
	    return(-1);
...
xmlFARegExecSave(xmlRegExecCtxtPtr exec) {
	...
	exec->maxRollbacks = 4;
	exec->rollbacks = malloc(exec->maxRollbacks * sizeof(xmlRegExecRollback));
	...
	if (exec->rollbacks[exec->nbRollbacks].counts == NULL) {
	    exec->rollbacks[exec->nbRollbacks].counts = malloc(exec->comp->nbCounters * sizeof(int));
	    if (exec->rollbacks[exec->nbRollbacks].counts == NULL) {
		xmlRegexpErrMemory(NULL, "saving regexp");
		exec->status = -5;
		return;
...
xmlFARegExec(xmlRegexpPtr comp, const xmlChar *content) {
	...
	exec->counts = malloc(comp->nbCounters * sizeof(int));
	if (exec->counts == NULL) {
...
xmlRegNewExecCtxt(xmlRegexpPtr comp, xmlRegExecCallbacks callback, void *data) {
    ...
    if ((comp->compact == NULL) && (comp->states == NULL))
        return(NULL);
    exec = malloc(sizeof(xmlRegExecCtxt));
    if (exec == NULL) {
	 ...
	 */
	exec->counts = malloc(comp->nbCounters * sizeof(int) * 2);
	if (exec->counts == NULL) {
	    xmlRegexpErrMemory(NULL, "creating execution context");
	    free(exec);
	    return(NULL);
...
xmlFARegExecSaveInputString(xmlRegExecCtxtPtr exec, const xmlChar *value,
	...
	exec->inputStack = malloc(exec->inputStackMax * sizeof(xmlRegInputToken));
	if (exec->inputStack == NULL) {
...
xmlExpExpDeriveInt(xmlExpCtxtPtr ctxt, xmlExpNodePtr exp, xmlExpNodePtr sub) {
    ...
    tab = malloc(ctxt->tabSize * sizeof(const xmlChar *));
    if (tab == NULL) {
	return(NULL);
    }
    ...
    while (len < 0) {
        const xmlChar **temp;
	temp = realloc((xmlChar **)tab,
                          ctxt->tabSize * 2 * sizeof(const xmlChar *));
	if (temp == NULL) {
	    free(tab);
	    return(NULL);
	}
