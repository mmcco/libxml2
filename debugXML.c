 ...
 */
static void
xmlCtxtCheckString(xmlDebugCtxtPtr ctxt, const char * str)
{
    if (str == NULL) return;
    if (ctxt->check) {
        if (!xmlCheckUTF8(str)) {
 ...
 */
static void
xmlCtxtCheckName(xmlDebugCtxtPtr ctxt, const char * name)
{
    if (ctxt->check) {
	if (name == NULL) {
...
xmlCtxtGenericNodeCheck(xmlDebugCtxtPtr ctxt, xmlNodePtr node) {
    ...
    if ((node->parent != NULL) && (node->doc != node->parent->doc) &&
        (!xmlStrEqual(node->name, "pseudoroot")))
    ...
    switch (node->type) {
	    ...
	    if ((ctxt->dict != NULL) &&
	        (node->name == xmlDictLookup(ctxt->dict, "nbktext",
		                             7)))
		break;
...
static void
xmlCtxtDumpString(xmlDebugCtxtPtr ctxt, const char * str)
{
    int i;
...
xmlCtxtDumpOneNode(xmlDebugCtxtPtr ctxt, xmlNodePtr node)
{
    ...
    switch (node->type) {
        ...
        case XML_TEXT_NODE:
            if (!ctxt->check) {
                xmlCtxtDumpSpaces(ctxt);
                if (node->name == (const char *) xmlStringTextNoenc)
 ...
 */
void
xmlDebugDumpString(FILE * output, const char * str)
{
    int i;
      ...
      xmlNodePtr root, xmlNodePtr node2 ATTRIBUTE_UNUSED)
{
    ...
    while (ns != NULL) {
        if (ns->prefix == NULL)
	    xmlXPathRegisterNs(ctxt->pctxt, "defaultns", ns->href);
	else
            ...
            char *arg, xmlNodePtr node, xmlNodePtr node2 ATTRIBUTE_UNUSED)
{
    ...
    while (node != NULL) {
	    ...
	    if (xmlStrstr(node->content, (char *) arg)) {
...
#endif /* LIBXML_XPATH_ENABLED */
        ctxt->filename = (char *) xmlCanonicPath((char *) filename);
    } else
        return (-1);
    return (0);
}
                 ...
                 xmlNodePtr node2 ATTRIBUTE_UNUSED)
{
    ...
    if ((dtd == NULL) || (dtd[0] == 0)) {
        ...
        subset = xmlParseDTD(NULL, (char *) dtd);
        if (subset != NULL) {
         ...
         FILE * output)
{
    ...
    if (output == NULL)
        output = stdout;
    ctxt = (xmlShellCtxtPtr) malloc(sizeof(xmlShellCtxt));
    if (ctxt == NULL)
        return;
    ctxt->loaded = 0;
    ctxt->doc = doc;
...
#ifdef LIBXML_XPATH_ENABLED
                ctxt->pctxt->node = ctxt->node;
                list = xmlXPathEval((char *) arg, ctxt->pctxt);
#else
                list = NULL;
#endif /* LIBXML_XPATH_ENABLED */
                if (list != NULL) {
